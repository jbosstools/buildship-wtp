...
public class WebApplicationConfigurator implements IProjectConfigurator {
    ...
    private void installWebFacet(Set<Action> actions, IProject project, IFacetedProject facetedProject, String projectPath) {
        ...
        IProjectFacetVersion webFacetVersion = getWebFacetVersion(warModel);

        if (!facetedProject.hasProjectFacet(WebFacetUtils.WEB_FACET)) {
            actions.add(new IFacetedProject.Action(IFacetedProject.Action.Type.INSTALL, webFacetVersion, null));
        } else {

        }
    }

    private IProjectFacetVersion getWebFacetVersion(WarModel warModel) {
        return null;
    }
